...
public class Abordagem1 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < instancias.length; i++) {
            ...
            String ans = prefixoComum1(arrayAtual);
    ...
    static String prefixoComum1(String arr[]) {
        int n = arr.length;
        String comparacao = arr[0];

        for (int i = 1; i < n; i++) {
            int minLength = Math.min(comparacao.length(), arr[i].length());
            int index = 0;

            while (index < minLength && comparacao.charAt(index) == arr[i].charAt(index)) {
                index++;
            }
